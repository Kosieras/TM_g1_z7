...
<br> 
   ...
   <form action="logout.php"><br> <input type="submit" value="LOGOUT"/></form><br><br><br>
    ...
    <form name="formAdd" id="formAdd" method="POST" action="upload.php" enctype="multipart/form-data"> <div class="up" id="up" style="visiblity:hidden; position:absolute;">
    	<input type="text" name="title" id="title" value="Tytul"><br>
      	<input type="text" name="director" id="director" value="Rezyser"><br>
      	<input type="text" name="subtitles" id="subtitles" value="Napisy" style="line-height: 4em;"><br>
        <select name="user_idmt"><?php
    		$odbiorca = mysqli_query($connection, "Select * from filmtype") or die ("DB error: $dbname");
     	...
     	<input type="file" name="fileToUpload" id="fileToUpload" accept="video/*"><br>
   ...
   <label id="playlist" for="addPlaylist">
    ...
    <!--Przycisk do powrotu -->
    <form method="POST" action="mynetflix.php" enctype="multipart/form-data">
	...
	<form id="aP" class="aP" method="POST" action="mynetflix.php" enctype="multipart/form-data">
    ...
    <div id="addTo" style="visiblity:hidden; position:absolute;">
        ...
        <select name="playlistValue"><?php
   			$odbiorca = mysqli_query($connection, "Select * from film") or die ("DB error: $dbname");
    		while ($row = mysqli_fetch_array ($odbiorca)){
...
<?php
   //Tabela od wyswietlania playlist
    ...
    $findPlaylist = mysqli_query($connection, "Select * from playlistname") or die ("DB error 2: $dbname");
 	while ($row = mysqli_fetch_array ($findPlaylist)) if($row[1] == $_SESSION['userid'])  print "<TR><TD><a href='mynetflix.php?playlist=$row[0]'>$row[2]</a></TD></TR> ";
    ...
    print "<TR><TD><a href='mynetflix.php?playlist=$row[0]'>$row[2]</a></TD></TR> ";
    //Zmienne
    ...
    if(isset($subToPlaylist))
    	$addToPlaylist = mysqli_query($connection, "INSERT INTO playlistdatabase (idpl, idf) values ('$playlist', '$playlistValue') ") or die ("DB error 2:  $connection->error);");
	$findPlaylist2 = mysqli_query($connection, "Select * from playlistname") or die ("DB error 2: $dbname");
	 while ($row3 = mysqli_fetch_array ($findPlaylist2)) {
   			...
   			if(!isset($playlist)) $playlistNow = "WSZYSTKIE FILMY";
 }
	print "<br><br><TABLE id='tab' class='tab' CELLPADDING=5 BORDER=1>";
	print "<TR><TD style='color:red'><b>$playlistNow</b></TD><TD>Tytuł</TD><TD>Rezyser</TD><TD>Gatunek</TD></TR>\n"; 
	$dir = "films/";
	$files = array_diff(scandir($dir), array('.', '..'));
    if(!isset($_GET['playlist'])){
    $findP = mysqli_query($connection, "Select * from film join filmtype on film.idft = filmtype.idft;") or die ("DB error 2:  $connection->error);");
    while($row = mysqli_fetch_array ($findP))
    	print "<TR><TD> <video controls src='$dir/$row[5]'></video></TD><TD><a href='$dir/$row[5]' download>$row[1]</a></TD><TD>$row[2]</TD><TD>$row[9]</TD></TR>\n";
    }
    else {
   	$findP = mysqli_query($connection, "Select * from film join filmtype on film.idft = filmtype.idft join playlistdatabase on film.idf = playlistdatabase.idf where (playlistdatabase.idpl='$playlist')") or die ("DB error 2:  $connection->error);");
    while($row = mysqli_fetch_array ($findP))
    	print "<TR><TD> <video controls src='$dir/$row[5]'></video></TD><TD><a href='$dir/$row[5]' download>$row[1]</a></TD><TD>$row[2]</TD><TD>$row[9]</TD></TR>\n";
 	}
	print "</TABLE>"; 
